...
export default function Exercicios() {
  ...
  // -----------------------------
  // CORRIGIDO: CARREGAR DA TURMA
  // -----------------------------
useEffect(() => {
  if (!turma || turma === "selecione uma turma") return;

  const storageKey = `Exercícios do ${turma}`;
  const saved = localStorage.getItem(storageKey);

  if (saved && JSON.parse(saved).length > 0) {
    // Se já tem coisas salvas, carrega
    setExercises(JSON.parse(saved));
  } else {
    // Se não tem nada salvo, NÃO sobrescreve com a lista padrão
    // Apenas cria a chave vazia e mantém os exercises atuais como estão
  ...
  const handleGerar = async () => {
    ...
    try {
      ...
      const uniqueCard = {
        ...
        description: output,
        summary: output.slice(0, 200) + "...", // CORRIGIDO: prévia
        answer: "",
