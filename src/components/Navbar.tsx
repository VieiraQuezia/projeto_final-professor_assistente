      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center gap-6">
            {links.map(link => <Link key={link.to} to={link.to} className={`text-sm font-medium transition-colors ${isActive(link.to) ? 'text-[#1E88E5] border-b-2 border-[#1E88E5] pb-1' : 'text-gray-700 hover:text-[#1E88E5]'}`}>
                {link.label}
              </Link>)}
          </div>
        ...
        {isOpen && <div className="md:hidden py-4 border-t">
            {links.map(link => <Link key={link.to} to={link.to} onClick={() => setIsOpen(false)} className={`block py-2 px-4 text-sm font-medium transition-colors ${isActive(link.to) ? 'text-[#1E88E5] bg-blue-50' : 'text-gray-700 hover:bg-gray-50'}`}>
                {link.label}
              </Link>)}
          </div>}
      </div>
